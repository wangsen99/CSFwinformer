...
def get_normalized_predict_mask(mask):
    mask = np.trunc(mask * 255)
    if np.max(mask) > 0:
        mask = (mask - np.min(mask))/(np.max(mask) - np.min(mask))
    mask = mask.astype(np.float32)
    return mask

def get_binary_predict_mask(mask):
...
def get_eval(results, gt_seg_maps):
    ...
    for i in tqdm(range(num_imgs)):
        ...
        predict_normalized = get_normalized_predict_mask(pred)
        predict_binary = get_binary_predict_mask(pred)
        


        acc = accuracy_mirror(predict_binary, gt)
        iou = compute_iou(predict_binary, gt)
        precision, recall = cal_precision_recall((predict_normalized * 255).astype(np.uint8), (gt * 255).astype(np.uint8))
        for idx, data in enumerate(zip(precision, recall)):
            p, r = data
            precision_record[idx].update(p)
            recall_record[idx].update(r)
        mae = compute_mae(predict_normalized, gt)
        ber = compute_ber(predict_binary, gt)
